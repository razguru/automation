...
if [[ $region != all ]]; then gp2_vol_id="$region"_"$dt"_gp2_vol_id.txt
fi
...
auto-disc()
{
gp2_vol_id="$region"_"$dt"_gp2_vol_id.txt
gp2_vol_error="$region"_"$dt"_gp2_vol_error.txt
...
if [ -s "$gp2_vol_id" ]; then 
    ...
    if [ $? -eq 0 ] && [ "${result}" == "modifying" ];then
        echo "`date +%F-%T` SUCCESS: volume ${vol_id} changed to GP3 .. state 'modifying'" |tee -a "$region"_"$dt"_modify_vol.log
	echo ""
    else
        echo -n "`date +%F-%T` ERROR: couldn't change volume ${vol_id} type to GP3!" |tee -a "$region"_"$dt"_modify_vol.log
        echo ""
	exit 1
    fi
done
...
echo ""
echo "GP2-GP3 volume modification logs : "$region"_"$dt"_modify_vol.log"
echo "~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~"
echo ""
fi
}
...
if [ -s "$gp2_vol_id" ]; then
	...
	/usr/bin/aws ec2 create-snapshot --region "${region}" --volume-id "${vol_id}" --description 'Pre GP3 migration' --tag-specifications 'ResourceType=snapshot,Tags=[{Key=state,Value=pre-gp3}]' &>> "$region"_"$dt"_snapshot_vol.log
	if [ $? -ne 0 ]; then
	       echo ERROR: "snapshot creation failed for volume_id $vol_id, skipping this volume, check "$region"_"$dt"_snapshot_vol.log"
       else
	...
	snapshot_id=$(/usr/bin/aws ec2 describe-snapshots --region "${region}" --filters Name=volume-id,Values="${vol_id}" Name=tag:state,Values=pre-gp3 --query "reverse(sort_by(Snapshots, &StartTime))[0].SnapshotId")
        echo "`date +%F-%T` SUCCESS: snapshot $snapshot_id completed for  "${vol_id}"" |tee -a "$region"_"$dt"_snapshot_vol.log
	...
	echo "~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~"
        ...
        echo "`date +%F-%T` ERROR: could not change volume ${vol_id} type to GP3!" |tee -a "$region"_"$dt"_modify_vol.log
        echo ""
    fi
	fi
done
...
echo "GP2 volume-ids list : $gp2_vol_id"
echo ""
echo "Snapshot logs before GP3 migration : "$region"_"$dt"_snapshot_vol.log"
echo ""
echo "GP2-GP3 volume modification logs : "$region"_"$dt"_modify_vol.log"
echo "~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~*~~~~~~"
echo ""
